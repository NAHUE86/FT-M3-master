...
server.post("/posts", (req,res)=>{
    ...
    if( !author || !title || !contents ){
        return res.status(STATUS_USER_ERROR).json ({error:"no se recibieron parámetros necesarios para crear el Post"})
    }
    const newPost ={
        id: nextId,
        author,
...
server.delete(`/author`, (req, res)=>{
    ...
    if(!author){
        return res.status(STATUS_USER_ERROR).json({error: "Te olvidaste el author"})
    }
